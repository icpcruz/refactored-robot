...
def write_artifact(artifacts_root: Path, payload: Dict[str, Any]) -> Path:
    ...
    artefact = artifacts_root / f"ingest_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
...
def main() -> None:
    ...
    if report_path and report_path.exists():
        report_meta = parse_report(report_path)
    else:
        report_meta = {"report_path": None, "items": [], "summary": "No report provided"}
    ...
    if not items:
        tid = insert_task(
            ...
            payload={"summary": report_meta.get("summary"), "report_path": report_meta.get("report_path")},
        )
